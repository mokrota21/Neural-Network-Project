...
# class NeuralNetwork:
#     def __init__(self, layers, rate, gamma=0.001) -> None:
#         self.alpha_ = rate
#         self.layers_ = layers
#         self.gamma_ = gamma
...
#             layer = self.layers_[i]
#             x = outputs[i]
#             de = layer.backward(de, x, self.alpha_, self.gamma_)
#         return mse
...
class NeuralNetwork:
    ...
    def backward(self, outputs, y_real):
        ...
        for i in reversed(range(len(self.layers))):
            ...
            if uniform(0,1) > self.gamma:
                de = layer.backward(de, x, self.alpha, self.gamma)
            else:
                de = layer.derrivative(de)
    ...
    def train(self, x_set, y_set, epochs=1):
        assert len(x_set) == len(y_set)
        ...
        for epoch in range(epochs):
            for i in range(len(x_set)):
                ...
                outputs = self.forward(x)
                # print(outputs)
                # print(f"MSE: {self.backward(outputs, y_real)};{' ' * 20}Learning rate: {self.alpha}")
                avg_mse += self.backward(outputs, y_real)
            print(f"Epoch: {epoch}\nAvg MSE: {avg_mse / len(x_set)}{' ' * 20}Learning rate: '{self.alpha}'")
            shuffle(self.layers)
